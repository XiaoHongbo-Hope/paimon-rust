...
def expected_sdist_pyproject() -> bytes:
...
def verify_wheel(path: Path) -> tuple[str, str]:
    ...
    with archive:
        ...
        require(f"{dist_info}/RECORD" in names, f"wheel has no RECORD: {path.name}")
            ...
            for member in members:
                ...
                if relative.name == "THIRD-PARTY-LICENSES.html":
                    ...
                    markers = (
                        ...
                        b"<code>bindings/python/Cargo.toml</code>",
                    )
                    for marker in markers:
                        require(
                            marker in actual,
                            f"wheel license report is missing marker {marker!r}",
                        )
...
def verify_sdist(path: Path) -> str:
    ...
    with archive:
        ...
        require(
            read_member(f"{root}/pyproject.toml") == expected_sdist_pyproject(),
