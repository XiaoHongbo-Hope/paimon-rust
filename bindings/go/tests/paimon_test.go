...
func openCopiedTestTable(t *testing.T) *paimon.Table {
	return openCopiedTable(t, "simple_log_table")
}
...
func TestWritePostponeTableUsesVisibleFixedBuckets(t *testing.T) {
	table := openCopiedTable(t, "postpone_bucket_pk_table")
	...
	messages, err := write.PrepareCommit()
	if err != nil {
		t.Fatalf("Failed to prepare postpone commit: %v", err)
	}
	defer messages.Close()
	commit, err := builder.NewCommit()
	if err != nil {
	...
	defer commit.Close()
	if err := commit.Commit(messages); err != nil {
		t.Fatalf("Failed to commit postpone write: %v", err)
	}

	rows := readTableRows(t, table)
	if len(rows) != 1 || rows[0] != (row{4, "dave"}) {
		t.Fatalf("Expected the new fixed-bucket row to be immediately visible, got: %v", rows)
	}
}
